...
import static me.livanec.don.tic_tac.R.*;
...
public class BoardView extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		background.setColor(getResources().getColor(color.board_background));
		canvas.drawRect(0, 0, getWidth(), getHeight(), background);

		Paint dark = new Paint();
		dark.setColor(getResources().getColor(color.board_dark));

		Paint hilite = new Paint();
		hilite.setColor(getResources().getColor(color.board_hilite));

		Paint light = new Paint();
		light.setColor(getResources().getColor(color.board_light));

		Paint winner = new Paint();
		winner.setColor(getResources().getColor(color.board_hint_0));
		...
		foreground.setColor(getResources().getColor(color.board_foreground));
		...
		if (game.getUnselectedCount() < 9) {
			...
			selected.setColor(getResources().getColor(color.board_selected));
	...
	public void setSelectedTile(int tile) {
		if (game.setSquareIfValid(selX, selY, tile)) {
			...
			startAnimation(AnimationUtils.loadAnimation(game, anim.shake));
		}
	}
